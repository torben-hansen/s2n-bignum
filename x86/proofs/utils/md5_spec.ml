...
(*                                                                           *)
(* Pure functional definition, hardware-independent; the ground truth for    *)
(* the x86-64 md5_block_asm_data_order correctness proof.  The chaining      *)
(* state is an `int32 list` of length 4 holding [A;B;C;D] -- a list rather   *)
(* than a tuple so EL indexing and LENGTH lemmas apply and the four working  *)
(* words sit at stable positions across cut points.                          *)
(*                                                                           *)
(* The K / rotate / message-index tables are transcribed in round order      *)
(* from RFC 1321, cross-checked against the KAT-validated C reference in     *)
(* tests/test_md5.c (itself differential-tested against the frozen assembly *)
(* and the published RFC digests in the Phase-1 gate).  Round groups:        *)
